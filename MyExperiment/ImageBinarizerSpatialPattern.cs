...
namespace NeoCortexApiSample
{
    internal class ImageBinarizerSpatialPattern
    {
        ...
        private SpatialPooler RunExperiment(HtmConfig cfg, string inputPrefix)
        {
            ...
            string trainingFolder = _directoryPath;
            ...
            while (!isInStableState && currentCycle < maxCycles)
            {
                foreach (var Image in trainingImages)
                {
                    ...
                    string inputBinaryImageFile = NeoCortexUtils.BinarizeImage($"{Image}", imgSize, _directoryPath);
        ...
        private void RunRustructuringExperiment(SpatialPooler sp)
        {
            ...
            // Define the folder path for difference heatmaps
            string differenceHeatmapFolderPath = Path.Combine(Environment.CurrentDirectory, "DifferenceHeatmapImages_ImageInput");

            // Generate the Difference Heatmaps using the normalized and encoded data
            NeoCortexUtils.GenerateDifferenceHeatmaps(normalizedPermanence, BinarizedencodedInputs, differenceHeatmapFolderPath, 100, 1, 30);
        }
        ...
        private void Generate1DHeatmaps(List<List<double>> heatmapData, List<int[]> normalizedPermanence, List<int[]> BinarizedencodedInputs)
        {
            ...
            foreach (var values in heatmapData)
            {
                ...
                Console.WriteLine("Heatmap generated and saved successfully.");
                i++;
            }
        ...
        public static void DrawSimilarityPlots(List<double[]> similaritiesList)
        {
            ...
            Console.WriteLine($"Combined similarity plot generated and saved successfully.");
